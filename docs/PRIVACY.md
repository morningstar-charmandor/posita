...
consistency preflight. The typed challenge text is kept only in memory. Only then may the trusted
...
and no provider mailbox is contacted or changed. Because neither producer nor
recovery command has IPC, UI, or startup composition, this policy cannot currently
delete data in the running application.
