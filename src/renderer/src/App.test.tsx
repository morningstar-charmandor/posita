...
describe('Posita vertical slice', () => {
  ...
  it('shows reviewed Gmail consent without activating OAuth or implying live accounts', async () => {
    ...
    expect(screen.getByRole('list', { name: 'Google permissions requested' }))
      .toHaveTextContent('https://www.googleapis.com/auth/gmail.readonly')
    expect(dialog).toHaveTextContent('openid')
    expect(dialog).toHaveTextContent('Verified Google account identity')
