...
implements AccountConnectionRecoveryConfirmationVerifier {
  ...
  isValid(request: RecoverAccountConnectionRequestV1): boolean {
    const record = this.load(request.confirmationId)
    return record !== undefined && this.matchesRequest(record, request) &&
      Date.parse(record.expiresAt) >= this.validNow().getTime()
  }
