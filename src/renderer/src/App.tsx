...
import type { Account, BriefItem, Message, Topic } from '@shared/domain'
...
const accountById = (id: string): Account => fixtures.accounts.find((account) => account.id === id)!
const senderName = (message: Message): string => fixtures.people.find((person) => person.id === message.senderId)?.name ?? 'Unknown'
  ...
  return (
    <aside className="sidebar">
      ...
      <nav aria-label="Primary">
        ...
        {fixtures.accounts.map((account) => <button className="account-link" key={account.id}><span className={`account-dot account-${account.tone}`} /><span>{account.label}</span></button>)}
      </nav>
...
function TopicView({ topic, onBack, onOpenMessage, onDraft, onAsk }: { topic: Topic; onBack: () => void; onOpenMessage: (id: string) => void; onDraft: () => void; onAsk: (query: string) => void }): React.JSX.Element {
  const people = topic.participantIds.map((id) => fixtures.people.find((person) => person.id === id)!).filter(Boolean)
  return (
    <main className="center-pane">
...
export function App(): React.JSX.Element {
  ...
  const topic = useMemo(() => view.kind === 'topic' ? getTopic(fixtures, view.topicId) : undefined, [view])
  const draftTopic = draftTopicId ? getTopic(fixtures, draftTopicId) : undefined
